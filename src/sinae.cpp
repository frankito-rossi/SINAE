...
SINAE cargarDatos(SINAE &S){
 ...
 SINAE cargar_vacunas(SINAE &S, Vacunas v, int cantV, char* region){
     ...
     if(S->arrSECOED[0]->zona == region){
    	 printf("La region es %s \n", region);
         i = 0;
     }
     if(S->arrSECOED[1]->zona == region){
    	 printf("La region es %s \n", region);
    	 i = 1;
     }
     if(S->arrSECOED[2]->zona == region){
    	 printf("La region es %s \n", region);
    	 i = 2;
     }
     if(S->arrSECOED[3]->zona == region){
    	 printf("La region es %s \n", region);
    	 i = 3;
     }
 ...
 char* agendar(SINAE &S, Fecha f, char* locPersona){
	 ...
	 int i = 0;
	 printf("Antes de perteneceARegion loc = %c. \n", locPersona);
	 bool flag = true;
	while(flag){
		printf("region = %s \n", S->arrSECOED[i]->zona);
		printf("locPersona = %s", locPersona);
		Grafo g = S->arrSECOED[i]->grafo;
		flag = !(perteneceARegion(g, locPersona));
		i++;
	}
	while(res == "" && i<4){
		PuntoVacunacion p = buscar_lugarConVacunas(S->arrSECOED[i]->grafo, locPersona);
		res = localidad(p);
		agendar(p, f);
